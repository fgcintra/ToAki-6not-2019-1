...
export class LeitorQrCodeComponent implements OnInit {
  ...
  constructor(private snackBar: MatSnackBar, private location: Location, private rota : Router) {}
  ...
  handleQrCodeResult(resultString: string) {
      ...
      let oficina = JSON.parse(resultString);

      let {cod_oficina} = oficina;

       let codigo_oficina_selecionada = JSON.parse(localStorage.getItem('oficina'));

      if(cod_oficina == codigo_oficina_selecionada.cod_oficina) {

        alert("cadastrado com sucesso");
        this.rota.navigate(['events']);

      }
      else{

        alert("QR Code não bate");
        this.rota.navigate(['events']);
      };
  }
